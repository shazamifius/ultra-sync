...
impl LogEntry {
    ...
    pub fn calculate_hash(&self) -> Vec<u8> {
        ...
        data_to_hash.extend_from_slice(&self.timestamp.to_rfc3339().as_bytes());
        data_to_hash.extend_from_slice(&self.peer_id);
        data_to_hash.extend_from_slice(&bincode::serialize(&self.event_type).unwrap());
        data_to_hash.extend_from_slice(&self.payload);
        data_to_hash.extend_from_slice(&self.prev_hash);
...
impl Ledger {
    ...
    pub fn load<P: AsRef<Path>>(path: P) -> io::Result<Self> {
        ...
        if entries.is_empty() {
            ...
            bincode::serialize_into(&mut writer, &entries[0]).unwrap();
        }
    ...
    pub fn append_entry(&mut self, peer_id: Vec<u8>, event_type: EventType, payload: Vec<u8>) -> io::Result<()> {
        let prev_hash = self.get_last_hash();
        let new_entry = LogEntry::new(peer_id, event_type, payload, prev_hash);

        let file = OpenOptions::new().append(true).open(&self.path)?;
        let mut writer = BufWriter::new(file);
        bincode::serialize_into(&mut writer, &new_entry).unwrap();
    ...
    pub fn verify_integrity(&self) -> bool {
        ...
        if self.entries[0].self_hash != self.entries[0].calculate_hash() {
            return false;
        }
        ...
        for i in 1..self.entries.len() {
            ...
            if current_entry.self_hash != current_entry.calculate_hash() {
                return false;
            }
        }
        true
...
mod tests {
    ...
    fn ledger_persistence_and_loading_works() {
        ...
        let peer1 = b"peer1".to_vec();
        ledger.append_entry(peer1.clone(), EventType::FileLockRequested, b"file1.txt".to_vec()).unwrap();
